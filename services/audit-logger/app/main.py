...
async def lifespan(app: FastAPI):
    logger.info("Starting AuditLogger service...")
    ...
    if EUREKA_AVAILABLE and settings.ENABLE_EUREKA:
        try:
            ...
            logger.info("✓ AuditLogger registered with Eureka")
    ...
    if eureka_registry:
        try:
            ...
            logger.info("✓ AuditLogger deregistered from Eureka")
        except Exception as e:
...
app = FastAPI(
    title="MedBot Intelligence: AuditLogger",
    description="Audit and Compliance Logging Service (HIPAA/GDPR)",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
@app.get("/")
async def root():
    return {"service": "AuditLogger", "version": "1.0.0", "status": "healthy"}
